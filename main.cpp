...
private:
	...
	public:
		friend class Mult<T>;
		friend class iterator;
		mult_node(T node_val) : val(node_val) {}
		mult_node() {}
		~mult_node() { next = nullptr; prev = nullptr; }
		void print_val()
		{
			std::cout << val << " ";
		}
...
public:
	...
	public:
		iterator() :the_node(0) {}
		iterator(mult_node* dn) : the_node(dn) {}
		...
		{
			if (the_node == 0)
				throw "Попытка уменьшит пустой итератор";
			if (the_node->prev == 0)
	...
	{
		mult_node* node_to_add = new mult_node(node_val);
	...
	{
		for (mult_node* dn = head; dn != tail; dn = dn->next)
		{
	...
	T remove_front() //удаление
	{
		if (is_empty())
		throw "Попытка удалить из пустого множества!";
		mult_node* node_to_remove = head;
	...
	T remove(const T& element)
	{
		...
		for (mult_node* node_to_remove = head; node_to_remove != tail; node_to_remove = node_to_remove->next)
		{
	...
	T remove_rear() 
	{
		...
		mult_node* node_to_remove = tail->prev;
