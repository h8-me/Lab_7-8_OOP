...
private:
	...
	public:
		...
		Double_node() {}
		~Double_node() { next = nullptr; prev = nullptr; }
		void print_val()
		{
			std::cout << val << " ";
		}
...
public:
	...
	T remove(const T& element)
	{
		...
		for (Double_node* node_to_remove = head; node_to_remove != tail; node_to_remove = node_to_remove->next)
		{
...
class Multitude
{
	Double_list<T> previous;

public:
	Double_list<T> multitude;
	Multitude() = default;
